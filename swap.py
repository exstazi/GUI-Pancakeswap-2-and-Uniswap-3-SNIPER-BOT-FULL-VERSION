...
try:
    ...
    class Uniswap:
        ...
        @check_approval
        def add_liquidity(
            self,gwei, my_address,my_pk, token: AddressLike, max_eth: Wei, min_liquidity: int = 1
        ) -> HexBytes:
            """Add liquidity to the pool."""
            tx_params = self._get_tx_params(value=max_eth, gwei=gwei)
            ...
            return self._build_and_send_tx(gwei, my_address,my_pk,function, tx_params)

        @supports([1])
        @check_approval
        def remove_liquidity(self, gwei, token: str, max_token: int) -> HexBytes:
            ...
            return self._build_and_send_tx(gwei, my_address,my_pk, my_address,my_pk,function)
        ...
        def make_trade(
            ...
            if input_token == ETH_ADDRESS:
                ...
                if output_token == ETH_ADDRESS:
                    ...
                    return self._token_to_token_swap_input(gwei,my_address,my_pk,
        ...
        def _eth_to_token_swap_input(
            ...
            if self.version == 1:
                ...
                tx_params = self._get_tx_params(value=qty, gwei=gwei)
        ...
        def _token_to_eth_swap_input(
            self,gwei, my_address,my_pk, input_token: AddressLike, qty: int, recipient: Optional[AddressLike]
        ) -> HexBytes:
            ...
            if self.version == 1:
                ...
                return self._build_and_send_tx(gwei, my_address,my_pk,function)
            else:
                if recipient is None:
                    recipient = self.address
                amount_out_min = int(
        ...
        def _eth_to_token_swap_output(
            self, gwei, my_address,my_pk,output_token: AddressLike, qty: int, recipient: Optional[AddressLike]
        ) -> HexBytes:
            ...
            if self.version == 1:
                ...
                tx_params = self._get_tx_params(value=eth_qty, gwei=gwei)
                func_params: List[Any] = [qty, self._deadline()]
                if not recipient:
                ...
                return self._build_and_send_tx(gwei, my_address,my_pk,
                    ...
                    self._get_tx_params(value=eth_qty, gwei=gwei),
                )

        def _token_to_eth_swap_output(
            self,gwei, my_address,my_pk, input_token: AddressLike, qty: Wei, recipient: Optional[AddressLike]
        ) -> HexBytes:
        ...
        def _token_to_token_swap_output(
            ...
            if self.version == 1:
                ...
                tx_params = self._get_tx_params(gwei=gwei)
                func_params = [
                    qty,
                    max_tokens_sold,
                    max_eth_sold,
        ...
        # ------ Approval Utils ------------------------------------------------------------
        def _get_tx_params2(self, value: Wei = Wei(0), gas: Wei = Wei(250000)) -> TxParams:
        ...
        def approve(self, token: AddressLike, max_approval: Optional[int] = None) -> None:
            ...
            print(f"Approving {_addr_to_str(token)}...")
            tx = self._build_and_send_tx2(function)
        ...
        def _build_and_send_tx(
            self,gwei, my_address,my_pk, function: ContractFunction, tx_params: Optional[TxParams] = None
        ) -> HexBytes:
            if not tx_params:
